...
from stage import dns, http, mitm, router
...
parser.add_argument(
    "--router", help="Router IP (Used for MITM ARP spoofing)", required=True
)
...
def main():
    ...
    t_mitm = threading.Thread(
        target=mitm.run, args=(opts.router, opts.target, opts.iface)
    )
    t_dns = threading.Thread(
        target=dns.run, args=(opts.router, opts.target, opts.iface)
    )
